...
<div class="inner-menu nav-collapse ">
    <div class="inner-wrapper">
        <a href="{{ action("client\MessageController@create") }}" class="btn btn-block btn-primary"><span
    ...
    <ul class="big-items message-tab">
        <li>
            <span class="badge badge-important">
                {{ count(App\Models\Message::where('to', Auth::user()->email)->where('owner', 1)->where('trashed', 0)->where('checked', 0)->get()) }}
            </span><a href="{{ action("client\MessageController@index") }}"> Inbox</a>
        </li>
        <li><a href="{{ action("client\MessageController@sent") }}">Sent</a></li>
        {{-- <li><a href="{{ action("client\MessageController@draft") }}">Draft</a></li> --}}
        <li><span class="badge badge-important">{{ count(App\Models\Message::where('trashed', 1)->where('owner', 1)
            ->Where(function($query){
                $query->Where('to', Auth::user()->email)
                    ->orWhere('from', Auth::user()->email);
            })->get()) }}
            </span><a href="{{ action("client\MessageController@trash") }}">Trash</a></li>
    </ul>
</div>
<!-- END SIDEBAR -->
