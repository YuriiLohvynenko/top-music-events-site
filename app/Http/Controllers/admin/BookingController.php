...
class BookingController extends Controller
{
    ...
    function save(Request $request) {
        ...
        $booking->datetime = ($request['date']) ? $request['date'].' '.$request['time'] : $request['time'];
        ...
        if(isset($request->add_on)){
            ...
            $priceLoc =  PriceLocation::find($request['location']);
        ...
        if (isset($request['date'])) {
            $booking->datetime1 = \DateTime::createFromFormat('m/d/Y h:i A',
                $request['date'].' '.$request['time'])->format('Y-m-d H:i:s');
        } else {

            $booking->datetime1 = \DateTime::createFromFormat('m/d/Y h:i A',
                $request['time'])->format('Y-m-d H:i:s');
        }
        $booking->save();
